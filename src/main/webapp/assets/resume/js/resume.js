...
let displayApplicant = function(applicant) {
    ...
    if (applicant.length > 0) {
        ...
        if (applicantData.email) {
            document.getElementById("new_applicant_email").innerHTML = applicantData.email;
        }
        if (applicantData.phone) {
            document.getElementById("new_applicant_phone").innerHTML = applicantData.phone;
        }
        if (applicantData.nationality) {
            document.getElementById("new_home_country").innerHTML = applicantData.nationality;
        }
    }
};
...
let renderSkills = function() {
	...
	for (let i = 0; i < skills.length; i++) {
		...
		let skillNameText = $("#skill-name option[value='" + object['skill-name'] + "']").text();
		let skillLevelText = $("#skill-level option[value='" + object['skill-level'] + "']").text();

		let listHtml = '<div class="m-widget4__item">'+
				'<div class="m-widget4__info">'+
					'<span class="m-widget4__title">'+
					skillNameText + ' | ' + skillLevelText +
					'</span><br> '+							 		 
				'</div>'+
				'<div class="m-widget4__ext m--align-right"> '+
				'<button onClick="editInit(this);" class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill m-edit" data-toggle="modal" data-target="#skillsModal" data-value="'+i+'" data-array="skills" title="Edit"><i class="la la-edit"></i></button> '+
...
let setEmploymentArray = function(dataArray) {
	if (!Array.isArray(dataArray)) {
        ...
        $('#errorContainer').html('<div class="alert alert-danger">Employment data is invalid or missing.</div>');
        return;
    }
