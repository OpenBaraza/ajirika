...
public class BResume extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		if ("getApplicant".equals(fnct)) {
			...
			// Get applicant array
			JSONArray applicants = jResp.optJSONArray("applicant");

			if (applicants == null || applicants.isEmpty()) {
				System.out.println("No applicant found. Adding applicant for entity_id: " + userID);
				addApplicant(userID);
				jResp = getApplicant(request);
			}
	...
	public void addApplicant(String userID) {
		try {
			// 1. Query entitys table for entity_name and primary_email
			String mySql = "SELECT entity_name, primary_email FROM entitys WHERE entity_id = '" + userID + "'";
			...
			if (rs.moveNext()) {
				...
				if (entityName != null && !entityName.trim().isEmpty()) {
					String[] parts = entityName.trim().split("\\s+"); // split by spaces
					if (parts.length >= 2) {
						firstName = parts[0];
						surname = parts[parts.length - 1]; // last element
					} else {
						// fallback if only one word exists
						firstName = entityName.trim();
						surname = entityName.trim();
					}
				}
			}
			...
			mData.put("surname", surname);
			mData.put("first_name", firstName);
			mData.put("applicant_email", primaryEmail);

			String inSql = "INSERT INTO applicants(entity_id, surname, first_name, applicant_email) VALUES (?,?,?,?)";
			db.saveRec(inSql, mData);
	...
	public JSONObject checkApplicant() {
		JSONObject jResp = new JSONObject();
		String primary_email = "";

		// Get entity details
		String mySql = "SELECT primary_email "
				+ "FROM entitys "
				+ "WHERE entity_id = " + userID;

		System.out.println("Checking entity email by entity_id: " + userID);
		...
		if (rs.moveNext()) {
			...
			System.out.println("Found primary Email: " + primary_email);

			JSONObject jData = new JSONObject();
			jData.put("applicant_email", rs.getString("primary_email"));
			aData.put(jData);
		}
		rs.close();

		// Put results into response (empty or with applicants)
		jResp.put("applicant", aData);

		System.out.println("From checkApplicant---------");
	...
	public JSONObject getApplicant(HttpServletRequest request) {
		...
		JSONObject checkResp =  new JSONObject();
		checkResp = checkApplicant();
		JSONArray applicants = checkResp.optJSONArray("applicant");
		...
		String applicantEmail = applicant.optString("applicant_email");
		...
		BQuery rs = new BQuery(db, checkedSql);
		while(rs.moveNext()) {
			JSONObject jData =  new JSONObject();
			...
			jData.put("marital_status", rs.getString("marital_status"));
			...
			jData.put("expected_salary", rs.getString("expected_salary"));
			jData.put("disability", rs.getString("disability"));
			jData.put("cv_data", rs.getString("cv_data"));
	...
	public JSONObject updateApplicant(HttpServletRequest request) {
		...
		mData.put("marital_status", request.getParameter("marital_status"));
	...
	public JSONObject updateSkill(HttpServletRequest request) {
		System.out.println("update skill reached in backend--------------------------------------------------------------");
		try {
			...
			e.printStackTrace();
			// return an empty or error JSON object instead of nothing
	...
	public JSONObject deleteProject(HttpServletRequest request) {		
		String projectId = request.getParameter("project_id");
		String delSql = "DELETE FROM cv_projects WHERE cv_project_id = " + projectId;
		db.executeFunction(delSql);
	...
	public JSONObject getProject(HttpServletRequest request) {
		JSONObject jResp =  new JSONObject();
		String mySql = "SELECT cv_project_id, cv_project_name, cv_project_date, details FROM cv_projects "
			+ "WHERE (org_id = " + orgId + ") AND (entity_id = " + userID + ")";
		BQuery rs = new BQuery(db, mySql);
		...
		while(rs.moveNext()) {
			JSONObject jData =  new JSONObject();
			jData.put("project_id", rs.getString("cv_project_id"));
